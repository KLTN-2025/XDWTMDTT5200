...
export default function VoucherTrash() {
  ...
  const fetchApi = async () => {
    const response = await listItem(token, "vouchers", currentPage, limit, keyword);
          ...
          <>
            <div>
              {permissions.includes("trashs_restore") && (
                <RestoreItem record={record} key={`restore-${record._id}`} typeDelete="voucher" onReload={handleReload} />
              )}
              {permissions.includes("trashs_permanent_del") && (
                <DeletePermanetItem record={record} key={`delete-permanent-${record._id}`} typeDelete="voucher" onReload={handleReload} />
              )}
            </div>
          </>
        )
