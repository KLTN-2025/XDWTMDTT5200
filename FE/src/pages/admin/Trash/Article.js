...
export default function ArticleTrash() {
  ...
  const fetchApi = async () => {
    const response = await listItem(token, "articles", currentPage, limit, keyword);
          ...
          <>
            <div>
              {permissions.includes("trashs_restore") && (
                <RestoreItem record={record} key={`restore-${record._id}`} typeDelete="article" onReload={handleReload} />
              )}
              {permissions.includes("trashs_permanent_del") && (
                <DeletePermanetItem record={record} key={`delete-permanent-${record._id}`} typeDelete="article" onReload={handleReload} />
              )}
            </div>
          </>
        )
