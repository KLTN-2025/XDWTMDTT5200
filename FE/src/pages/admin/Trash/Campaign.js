...
export default function CampaignTrash() {
  ...
  const fetchApi = async () => {
    const response = await listItem(token, "campaigns", currentPage, limit, keyword);
          ...
          <>
            <div>
              {permissions.includes("trashs_restore") && (
                <RestoreItem record={record} key={`restore-${record._id}`} typeDelete="campaign" onReload={handleReload} />
              )}
              {permissions.includes("trashs_permanent_del") && (
                <DeletePermanetItem record={record} key={`delete-permanent-${record._id}`} typeDelete="campaign" onReload={handleReload} />
              )}
            </div>
          </>
        )
